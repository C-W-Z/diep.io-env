...
class DeepSetEncoder(nn.Module):
    ...
    def forward(self, x):
        """
        No Batching. x: np.array
        """
        N, F = x.shape

        if N == 0:  # no objects in this category
            return torch.zeros(self.output_dim).to(x.device)

        features = []

        for i, meta in enumerate(self.feat_meta):
            col = x[:, i]
            if meta > 0:
                # Discrete: embed
                col = col.long()
        ...
        # Aggregate
        pooled = phi_out.sum(dim=0)  # (hidden_dim)
...
class EncoderWrapper(nn.Module):
    def __init__(self, device, feat_meta, embed_dim=8, hidden_dim=32, output_dim=64, method="deepset"):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if len(x) == 0:
            return torch.zeros(self.output_dim).to(self.dev)

        results = [self.encoder(torch.tensor(o, device = self.dev)) for o in x]
