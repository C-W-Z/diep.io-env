...
class DiepIOEnvBasic:
    ...
    def _get_obs(self, agent_id):
        ...
        obs = [
            ...
            agent.level - 1,                                # Player level [1, 45] -> [0, 44]
        ...
        for other_id, other_tank in enumerate(self.tanks):
            ...
            if min_x <= other_tank.x <= max_x and min_y <= other_tank.y <= max_y:
                tanks_obs.extend([
                    ...
                    other_tank.level - 1,                       # Level of the other tank [1, 45] -> [0, 44]
                ])

        # 4. Bullets
        self.bullet_feat_nb = 6
        bullet_obs = []
        ...
        return {
            ...
            "tanks": np.array(tanks_obs, dtype=np.float32).reshape(-1, self.tank_feat_nb),
            "polygons": np.array(polygon_obs, dtype=np.float32).reshape(-1, self.poly_feat_nb),
            "bullets": np.array(bullet_obs, dtype=np.float32).reshape(-1, self.bullet_feat_nb)
        }
...
from acting_agent import ActingAgent
...
if __name__ == "__main__":
    ...
    agent0 = ActingAgent(16 + 64 * 3, env.action_space, "saves/save_diep_agent0_18.bin")
    agent1 = ActingAgent(16 + 64 * 3, env.action_space, "saves/save_diep_agent1_18.bin")

    obs, _ = env.reset()

    while True:
        obs, rewards, dones, truncations, infos = env.step({
            "agent_0": agent0.act(obs["agent_0"]),
            "agent_1": agent1.act(obs["agent_1"]),
        })
        if dones["__all__"]:
            break
